...
import { ArrowLeft, Truck, Calendar, User, Award, MapPin, Package, CheckCircle2 } from 'lucide-react';
...
export function ManufacturerDispatchedOrders() {
  const navigate = useNavigate();
  const [batches, setBatches] = useState<DispatchedBatch[]>([]);
  ...
  useEffect(() => {
    const fetchBatches = async () => {
      try {
        ...
        const dispatchedBatches: DispatchedBatch[] = [];
        ...
        for (const [batchId, stages] of batchMap.entries()) {
          ...
          if (manufacturer && (status === 'Dispatched' || status === 'Shipped')) {
            ...
            dispatchedBatches.push({
              ...
              dispatchDate: new Date(manufacturer.timestamp).toLocaleDateString('en-GB'),
              status,
              quantity: `${farmer?.data?.quantityValue || '0'} ${farmer?.data?.quantityUnit || 'Kg'}`,
              farmerName: farmer?.data?.farmerName || 'Unknown Farmer',
              grade,
              destination: manufacturer?.data?.destination || 'Distribution Center'
            });
          }
        }

        dispatchedBatches.sort((a, b) => 
          new Date(b.dispatchDate.split('/').reverse().join('-')).getTime() - 
          new Date(a.dispatchDate.split('/').reverse().join('-')).getTime()
        );

        setBatches(dispatchedBatches);
        setStats({ 
          total: dispatchedBatches.length, 
          gradeA: gradeACount, 
          gradeB: gradeBCount, 
          gradeC: gradeCCount 
        });
    ...
    <div className='min-h-screen bg-gradient-to-br from-purple-50 to-indigo-100 p-6'>
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className='mb-6'>
        ...
        <div className='flex items-center gap-3 mb-2'>
          ...
          <div>
            <h1 className='text-3xl font-bold text-gray-800'>Dispatched Orders</h1>
            <p className='text-gray-600'>Products shipped to destinations</p>
          </div>
        </div>
      </motion.div>
      ...
      <div className='grid grid-cols-1 md:grid-cols-4 gap-4 mb-6'>
        {[
          { label: 'Total Dispatched', value: stats.total, color: 'purple', icon: Truck },
      ...
      {loading ? (
        ...
        <div className='bg-white rounded-2xl shadow-lg p-16 text-center'>
          ...
          <h2 className='text-2xl font-bold text-gray-900 mb-3'>No Dispatched Orders</h2>
          <p className='text-gray-600 text-lg'>Dispatched batches will appear here</p>
        </div>
      ) : (
        <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6'>
          {batches.map((batch, index) => (
            <motion.div
              ...
              className='bg-white rounded-xl p-6 shadow-md hover:shadow-xl transition-all cursor-pointer border-l-4 border-purple-500'
            >
              ...
              <div className='space-y-2 mb-4'>
                ...
                <div className='flex items-center gap-2 text-sm text-gray-600'>
                  ...
                  <span>Dispatched: {batch.dispatchDate}</span>
                </div>
                <div className='flex items-center gap-2 text-sm text-gray-600'>
