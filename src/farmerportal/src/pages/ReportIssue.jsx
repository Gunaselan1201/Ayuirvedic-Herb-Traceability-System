...
const ReportIssue = ({ onBack, farmerData, t }) => {
  ...
  const [formData, setFormData] = useState({
    ...
    issueTitle: '',
    description: '',
    addedBy: farmerData?.farmerName || 'Unknown'
  });
  ...
  const categories = [
    { value: 'QR Code Problem', label: 'QR Code Problem' },
    { value: 'Portal Bug', label: 'Portal Bug' },
    { value: 'Data Missing', label: 'Data Missing' },
    { value: 'Lab Testing Issue', label: 'Lab Testing Issue' },
    { value: 'Manufacturing Issue', label: 'Manufacturing Issue' },
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      formDataToSend.append('addedBy', formData.addedBy);

      if (selectedFile) {
        formDataToSend.append('attachment', selectedFile);
      }

      const response = await fetch('http://localhost:5174/api/issues', {
        method: 'POST',
        body: formDataToSend
      });
      ...
      // Show success toast
      setToastMessage(`✅ Issue reported successfully. Ticket ID: ${result.issueId}`);
      setShowToast(true);
        ...
        return (
          <button
            ...
            className={`flex-1 px-6 py-4 text-sm font-semibold transition-all relative flex items-center justify-center gap-2 ${
              currentStep === step.id
                ? 'text-green-700 border-b-3 border-green-600 bg-green-50'
                : currentStep > step.id
          ...
          <motion.button
            ...
            className={`p-6 rounded-xl border-2 transition-all shadow-md flex flex-col items-center justify-center min-h-[120px] w-full ${
              formData.category === cat.value
                ? 'border-green-600 bg-gradient-to-br from-green-50 to-green-100 shadow-xl ring-2 ring-green-200'
                : 'border-gray-200 bg-white hover:border-green-400 hover:shadow-lg'
            }`}
          >
            <div className={`text-base font-bold mb-2 text-center ${
              formData.category === cat.value ? 'text-green-700' : 'text-gray-900'
            }`}>
              {cat.label}
            </div>
            {formData.category === cat.value && (
    ...
    >
      ...
      <div>
        ...
        <input
          ...
          className="w-full px-4 py-3 border-2 border-gray-300 rounded-xl focus:ring-2 focus:ring-green-500 focus:border-green-500 bg-white transition-all"
        />
      </div>
      ...
      <div>
        ...
        <div className="border-2 border-dashed border-gray-300 rounded-xl p-6 text-center hover:border-green-500 hover:bg-green-50 transition-all bg-gray-50">
          {!selectedFile ? (
  ...
  const renderStep4 = () => (
    ...
    >
      ...
      <div className="bg-gradient-to-br from-green-50 to-blue-50 rounded-2xl p-6 space-y-4 border-2 border-green-200">
        <div className="flex items-start gap-3 pb-4 border-b-2 border-green-200">
      ...
      <div className="bg-gradient-to-r from-green-50 to-blue-50 border-2 border-green-300 rounded-xl p-4 shadow-md">
        <div className="flex items-start gap-3">
          <CheckCircle2 className="w-6 h-6 text-green-600 flex-shrink-0 mt-0.5" />
          <p className="text-sm text-gray-800">
            <strong className="text-green-700">Note:</strong> Once submitted, your issue will be assigned a unique ticket ID. 
    ...
    <div className="min-h-screen bg-gradient-to-br from-green-50 via-blue-50 to-purple-50">
      ...
      <AnimatePresence>
        {showToast && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -50 }}
            className="fixed top-20 right-4 z-50 bg-white rounded-lg shadow-2xl p-4 max-w-md border-l-4 border-green-600"
          >
            <div className="flex items-start gap-3">
      ...
      >
        ...
        <div className="bg-white rounded-2xl shadow-lg p-6 mb-6">
          <button
            onClick={onBack}
            className="flex items-center gap-2 text-gray-600 hover:text-green-700 mb-4 transition-colors font-medium"
          >
          ...
          <div className="flex items-center justify-between">
            ...
            <AlertCircle className="w-16 h-16 text-green-600 opacity-20" />
          </div>
        </div>
          ...
          <div className="flex justify-between mt-12 pt-6 border-t-2 border-gray-200 max-w-4xl">
            ...
            {currentStep < totalSteps ? (
              ...
              <motion.button
                ...
                className="flex items-center gap-2 px-10 py-3 bg-gradient-to-r from-green-600 to-blue-600 text-white rounded-xl font-bold hover:from-green-700 hover:to-blue-700 transition-all shadow-lg disabled:from-gray-400 disabled:to-gray-500 disabled:cursor-not-allowed"
              >
                {isSubmitting ? (
                  <>
